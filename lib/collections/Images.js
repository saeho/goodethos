...
var upload_limit = Meteor.settings && Meteor.settings.public && Meteor.settings.public.upload_limit ? Meteor.settings.public.upload_limit : 10300000
var small_upload_limit = Meteor.settings && Meteor.settings.public && Meteor.settings.public.small_upload_limit ? Meteor.settings.public.small_upload_limit : 3145728
...
if (Meteor.isClient) {
   ...
   */
  var sImg_stores =
    _.map(['smedium','ssmall','sthumb'], function(s){
   ...
   */
  var img_stores =
    _.map(['big','medium','small','thumb'], function(s){
      return Meteor.settings && Meteor.settings.AWS ?
        new FS.Store.S3(s) :
        new FS.Store.FileSystem(s)
    })
...
// Server
   ...
   */
   var sImg_stores = _.map(['smedium','ssmall','sthumb'], function(s){
     return Meteor.settings && Meteor.settings.AWS ?
       new FS.Store.S3(s, aws_define(s.substr(1), true)) :
       new FS.Store.FileSystem(s)
   })
   ...
   */
   var img_stores = _.map(['big','medium','small','thumb'], function(s){
     return Meteor.settings && Meteor.settings.AWS ?
       new FS.Store.S3(s, aws_define(s)) :
       new FS.Store.FileSystem(s)
   })
