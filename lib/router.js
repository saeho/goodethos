...
if( Meteor.isClient){
  ...
  var defRoute = {
    ...
    onStop: function(){
      if (stopSubs) stopSubs.stop() // stopSubs are sensitive subscriptions that must be stopped after each re-route
  ...
  Router.route('/blog', _.defaults({
    ...
    action: function() {
      // Profile Found
      var o_data = GE_Settings.findOne({ type: 'site_info' }) || {}
      var this_template = 'GE_blog'
      var meta = {
